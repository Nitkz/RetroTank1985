...
window.NesCHR = (function () {
  ...
  function drawExpandSprite(ctx, base, palIdx, dx, dy, scale = SCALE) {
    // 32x32 expand explosion sprite (16 tiles in a 4x4 layout from PT0)
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < 4; col++) {
        const tileAbs = base + (row & 1) + col * 2 + (row >> 1) * 8;
        drawCHRTile(ctx, tileAbs, palIdx, dx + col * 8 * scale, dy + row * 8 * scale, true, scale);
      }
    }
  }
  ...
  function prewarmTileCache() {
    ...
    // Spawn stars & Shield & Explosions (0xB0..0xB7, 0xD0..0xDF, 0xE0..0xEF, 0xF0..0xFB) & Power-ups (0x80..0x97) & Score (+500 PTS 0x3A..0x3D)
    const specialTiles = [
    ...
    for (let i = 0; i < specialTiles.length; i++) {
      const t = specialTiles[i];
      for (const pal of [0, 4, 5, 6, 7]) {
        getCachedCHRTile(t, pal, false, SCALE);
        getCachedCHRTile(t, pal, true, SCALE);
        getCachedCHRTile(256 + t, pal, false, SCALE);
        getCachedCHRTile(256 + t, pal, true, SCALE);
      }
    }
  }
