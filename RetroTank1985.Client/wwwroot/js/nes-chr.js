...
window.NesCHR = (function () {
  ...
  function prewarmTileCache() {
    ...
    // Spawn stars & Shield & Explosions & Power-ups (0x80..0x97) & Score (+500 PTS 0x3A..0x3D)
    const specialTiles = [
      ...
      0xA0, 0xA2, 0xA4, 0xA6, 0xAD, 0xA9, 0xA5, 0xA1
    ];
    for (let i = 0; i < specialTiles.length; i++) {
      getCachedCHRTile(specialTiles[i], 0, false, SCALE);
      getCachedCHRTile(specialTiles[i], 6, false, SCALE);
      getCachedCHRTile(specialTiles[i], 7, false, SCALE);
