...
function setChannelVol(chIdx, vol4bit) {
  ...
  let gain;
  if (chIdx === 2) {
    // Triangle: NES APU balanced volume (0.28 instead of 0.6)
    gain = vol4bit > 0 ? 0.28 : 0;
  } else if (chIdx === 3) {
    gain = (vol4bit / 15) * 0.35;
  } else {
    // Pulse 1 and Pulse 2: lead melodies
    gain = (vol4bit / 15) * 0.45;
  }
      ...
      if (b <= 0x5F) {
        ...
        if (s.pos + 1 < seq.length) {
          ...
          if (s.loopCtr[level] >= repeatCount) {
            ...
            // In ROM ($EC80-$EC9E): restartOff is already an absolute offset into the sequence data!
            s.pos = restartOff;
          }
        }
...
window.nesSynth = {
    ...
    playShot: function() { this.init(); sfxPlayerFire(); },
    playEnemyFire: function() { this.init(); sfxEnemyFire(); },
    playHitBrick: function() { this.init(); sfxBrickHit(); },
    playHitSteel: function() { this.init(); sfxSteelHit(); },
    playHitArmor: function() { this.init(); sfxArmorHit(); },
    playExplosion: function(isLarge) { this.init(); playSound(SND.EXPLOSION); if (isLarge) playSound(SND.ENTITY_KILL); },
    ...
    playTallyDone: function() { this.init(); sfxTallyDone(); },
    playPause: function() { this.init(); sfxPause(); },
    startEngine: function(isMoving) { this.init(); sfxStartEngine(); },
    updateEngineSpeed: function(isMoving) { this.init(); sfxStartEngine(); },
    stopEngine: function() { sfxStopEngine(); },
